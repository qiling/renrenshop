                ...
                <div class="fui-searchbar bar">
                    <div class="searchbar center">
                        ...
                        <div class="search-input">
                            ...
                            <input type="search" placeholder="搜索您感兴趣的内容..." class="search" value="<?php  echo $_GPC['keywords'];?>" name="keywords">
                        </div>
                    </div>
                </div>
            </form>
    ...
    <div class="fui-content navbar" style="padding-bottom: 0;">
        <div class="live-category" id="tab">
            <div class="live-category-content">
                <a href="javascript:void(0);" class="external <?php  if($cate==0) { ?>active<?php  } ?>" data-status="0">全部</a>
                <?php  if(is_array($categorys)) { foreach($categorys as $row) { ?>
                <a href="javascript:void(0);" class="external <?php  if($cate==$row['id']) { ?>active<?php  } ?>" data-status="<?php  echo $row['id'];?>"><?php  echo $row['name'];?></a>
                <?php  } } ?>
            </div>
        </div>
...
<script language='javascript'>
    require(['../addons/ewei_shopv2/plugin/live/static/js/list.js'], function (modal) {
        modal.init({page:1,cate:<?php  echo $cate;?>,keywords:'<?php  echo $keywords;?>'});
    });
</script>
